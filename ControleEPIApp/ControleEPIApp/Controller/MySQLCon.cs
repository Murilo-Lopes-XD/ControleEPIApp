...
namespace ControleEPIApp.Controller
{
    public class MySQLCon
    {
        ...
        public static void InserirFuncionario(string nome, string matricula, string epi, DateTime data_entrega, DateTime data_vencimento, double dias)
        {
        ...
        public static void AtualizarFuncionario(Funcionario func, double dias)
        {
            ...
            try
            {
                using (MySqlConnection con = new MySqlConnection(conn))
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(sql, con))
                    {
                        ...
                        cmd.Parameters.Add("@matricula", MySqlDbType.VarChar).Value = func.matricula;
                        cmd.Parameters.Add("@epi", MySqlDbType.VarChar).Value = func.epi;
                        func.data_entrega = DateTime.Today;
        ...
        public static void ExcluirPessoa(Funcionario func)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(conn))
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand(sql, con))
                {
                    cmd.Parameters.Add("@id", MySqlDbType.Int32).Value = func.id;
                    cmd.CommandType = CommandType.Text;
                    cmd.ExecuteNonQuery();
                }
                con.Close();
